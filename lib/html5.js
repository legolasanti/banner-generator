...
/**
 * Drop the @font-face blocks whose files this package does not carry.
 *
 * banner.css declares every family the tool uses — Arimo for Norsk Tipping and
 * ReadPeak, Noto Serif for Houseads — but a creative only ships the one its own
 * banners are set in, because a 200 KB budget cannot spare ~26 KB of unused
 * faces. The stylesheet goes into index.html verbatim, so the families that were
 * left out stay behind as `url("fonts/…woff2")` pointing at nothing.
 *
 * A browser never notices: it only fetches a face some text actually uses. The
 * ad server does. Campaign Manager 360 resolves every relative reference in the
 * bundle up front and REJECTS the upload on one it cannot find —
 *   "The HTML5 bundle contains a broken relative file reference(s):
 *    fonts/noto-serif-latin-400-normal.woff2, …"
 * — whether or not the rule is ever applied. So the CSS that ships has to
 * declare exactly what the ZIP contains, and nothing else.
 *
 * @param {string} css
 * @param {Array<{name:string}>} fonts  the faces actually going into fonts/
 * @returns {string}
 */
function pruneFontFaces(css, fonts) {
  const shipped = new Set((fonts || []).map((font) => font.name));
  // @font-face blocks carry no nested braces, so matching to the first "}" is
  // safe here and avoids pulling in a CSS parser for one rule type.
  return String(css).replace(/@font-face\s*\{[^{}]*\}\s*/g, (block) => {
    const urls = [];
    const pattern = /url\(\s*['"]?([^'")]+)['"]?\s*\)/g;
    let match;
    while ((match = pattern.exec(block)) !== null) urls.push(match[1]);
    // Keep the block only when every file it points at travels with it.
    const complete = urls.length > 0 && urls.every((url) => shipped.has(url.replace(/^.*\//, "")));
...
async function buildCreativeZip(params) {
  // Only declare the faces this ZIP carries — see pruneFontFaces().
  const css = pruneFontFaces(params.css, params.fonts);
  const files = [
    {
      name: "index.html",
      buffer: Buffer.from(buildIndexHtml(params.spec, params.markup, css, params.clickUrl, params.title), "utf8"),
    },
...
module.exports = { buildCreativeZip, zipToBuffer, creativeZipName, READ_ME, buildIndexHtml, pruneFontFaces };
