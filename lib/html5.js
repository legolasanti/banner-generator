...
VIL DU DOBBELTSJEKKE FØR OPPLASTING?
------------------------------------
Googles egen validator tar imot ZIP-filene direkte:
 ...
 * @param {{name:string,buffer:Buffer}=} params.icon        → optional custom mark
...
async function buildCreativeZip(params) {
  ...
  if (params.icon) files.push({ name: params.icon.name, buffer: params.icon.buffer });
