...
// In "auto", keep the lossless PNG when it is at most this much bigger than the
// JPEG. A little slack, because lossless is worth a few KB; a photo-sized gap
// is not.
const AUTO_PNG_TOLERANCE = 1.3;
...
/** Identify an image from its magic bytes. Returns a CM360 file extension. */
function sniffExtension(buffer) {
  if (!buffer || buffer.length < 12) return "";
  if (buffer[0] === 0xff && buffer[1] === 0xd8 && buffer[2] === 0xff) return "jpg";
  if (buffer[0] === 0x89 && buffer[1] === 0x50 && buffer[2] === 0x4e && buffer[3] === 0x47) return "png";
  if (buffer.slice(0, 3).toString("ascii") === "GIF") return "gif";
  return ""; // WEBP, AVIF, HEIC — none of them are CM360 display asset types
}

/**
 * What the photo will be called inside an HTML5 package.
 *
 * With sharp it is always image.jpg, because the photo is re-encoded. Without
 * sharp the upload ships byte-for-byte, so the name has to match what the file
 * actually is — a WEBP called image.jpg would be rejected by Campaign Manager
 * 360, which only accepts JPG, PNG and GIF here.
 *
 * @returns {{name: string, ok: boolean}} ok=false when the upload cannot be
 *   packaged at all without re-encoding.
 */
function photoAsset(buffer) {
  if (sharp) return { name: "image.jpg", ok: true };
  const ext = sniffExtension(buffer);
  return ext ? { name: "image." + ext, ok: true } : { name: "", ok: false };
}
...
async function encodeToBudget(png, opts) {
  ...
  if (format !== "jpeg") {
    ...
    // "auto" means smallest-that-still-looks-right, so a lossless PNG only wins
    // when it is actually competitive. On a photo it rarely is — a 98 KB PNG
    // against a visually equivalent 35 KB JPEG is not a trade worth making on
    // an ad server. On flat artwork the PNG is both smaller and sharper, and
    // this keeps it.
    if (format === "auto") {
      const jpeg = await source().jpeg(jpegOptions(Math.min(JPEG_CEIL, askedQuality))).toBuffer();
      const pngWins = lossless.length <= jpeg.length * AUTO_PNG_TOLERANCE;
      if (pngWins && fits(lossless)) return asPng(lossless);
      if (fits(jpeg)) return asJpeg(jpeg);
    } else if (fits(lossless)) {
      return asPng(lossless);
    }

    // ---- 2. 256-colour PNG, for someone who explicitly asked to stay on PNG.
    //         Still sharp on text; only a photo-heavy banner shows the banding,
    //         and it frequently lands well under budget. "auto" skips this and
    //         goes to JPEG, which looks better than a quantised photo at the
    //         same size.
    if (format === "png") {
...
async function preparePhoto(buffer, box, opts) {
  ...
  // No sharp: the upload ships untouched, so report the type it actually is
  // rather than claiming JPEG — the filename in the ZIP is built from this.
  if (!sharp) {
    const ext = sniffExtension(buffer) || "jpg";
    return {
      buffer,
      ext,
  ...
  // Decode the upload ONCE, into raw pixels at the largest size that could
  // ship. Every quality probe below then works from this small buffer instead
  // of re-decoding a 10 MB original — which, at up to two dozen probes per
  // format, would otherwise hold up the single render queue for a long time.
  const largest = PHOTO_SCALES[0];
  const base = await sharp(buffer)
    .rotate() // honour EXIF orientation before the metadata is dropped
    .resize(sizeFor(largest))
  ...
  const encodeAt = (factor, quality) => {
    const pipeline = sharp(base.data, { raw });
    if (factor !== largest) pipeline.resize(sizeFor(factor));
